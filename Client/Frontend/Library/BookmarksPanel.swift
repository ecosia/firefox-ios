...
class BookmarksPanel: SiteTableViewController, LibraryPanel {
    ...
    fileprivate func centerVisibleRow() -> Int {
        ...
        return profile.favourites.items.count
    }

    fileprivate func deleteBookmarkNodeAtIndexPath(_ indexPath: IndexPath) {
        tableView.beginUpdates()
        profile.favourites.remove(at: indexPath.row)
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        let page = profile.favourites.items[indexPath.row]
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        let bookmarkItem = profile.favourites.items[indexPath.row]
    ...
    func tableView(_ tableView: UITableView, moveRowAt sourceIndexPath: IndexPath, to destinationIndexPath: IndexPath) {
        profile.favourites.move(from: sourceIndexPath.row, to: destinationIndexPath.row)
    }
...
extension BookmarksPanel: LibraryPanelContextMenu {
    ...
    func getSiteDetails(for indexPath: IndexPath) -> Site? {
        guard let bookmarkNode = profile.favourites.items[safe: indexPath.row] else { return nil }
