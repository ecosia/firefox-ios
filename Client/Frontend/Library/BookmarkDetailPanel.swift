...
class BookmarkDetailPanel: SiteTableViewController {
    ...
    init(profile: Profile, bookmarkItemPosition: Int?) {
        ...
        if let index = bookmarkItemPosition, index < profile.favourites.items.count {
            let page = profile.favourites.items[index]
            bookmarkItemOrFolderTitle = page.title
            bookmarkItemURL = page.url.absoluteString
        }
    }
    ...
    func save() -> Error? {
        ...
        if isNew {
            profile.favourites.add(page)
        } else if let index = bookmarkItemPosition {
            profile.favourites.update(page, index: index)
        }
        return nil
    }
